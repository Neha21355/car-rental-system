...
A RESTful **Car Rental Management System backend** built with **Spring Boot**, **Spring Data JPA**, **Spring Security**, **MySQL**, **MapStruct**, **Bean Validation**, and **Jakarta Mail**.

The application supports two primary user types:

- **Car Owner** – registers, logs in, adds cars, views cars, and manages booking requests.
- **Customer** – registers, logs in, views available cars, and books a car.

The application uses HTTP sessions to keep track of logged-in customers and car owners.
...
└── test/
    └── java/
        └── com/ansari/car_rental_api_spring_boot_project/
```
...
If these paths differ after dependency/framework configuration changes, check the Springdoc documentation or application logs.
